...
1. **Le jour et l'heure du Selfty Call** (dans `config.json` : `jour` et `heure`). Valeurs actuelles = hypothèses : mardi 18:30.
...
`"jour": "jeudi"`, `"heure": "19:00"`. Les identifiants des événements restent stables : dans les agendas abonnés, les calls se déplacent, ils ne se dupliquent pas.
